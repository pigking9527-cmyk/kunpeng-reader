...
> 最新版本：Windows **v1.14.1** · macOS、Linux **v1.14.0** · Android **v0.3.0**。下载见 [Releases](https://github.com/pigking9527-cmyk/kunpeng-reader/releases)。v1.14.1 仅发布 Windows 包。
...
- **开启与管理**：进入“常用设置 → 手势”，打开右侧开关；点击齿轮可创建、搜索、启停、编辑或删除多条手势。录制时在画板上按住鼠标左键绘制，选择执行操作并测试、保存；实际使用时按住鼠标右键画出轨迹，松开后执行。
- **可选操作**：支持“返回／关闭当前页”“打开当前图书信息”“重新打开上一个页面”。手势会自动参与书架、阅读页、书库问答、资讯页、设置及其它弹窗的匹配；当前页面不支持所选操作时不会误执行。
- **识别方式**：轨迹会先消除位置和整体尺寸差异，再综合比较形状与八方向转折顺序；识别不要求总距离或每一段长度比例一致。例如录制“向下再向右”后，右移段较短也可以命中，不必为了补足长度再多画一段。仍需画出一段可辨识轨迹，以排除普通右键点击和手抖。
- **精度与提示**：匹配精度提供 1～10 档，可统一使用全局精度，也可为单条手势设置独立精度；档位越低越宽松，越高越严格。手势提示可独立开关，并调整文字大小、不透明度、背景颜色和屏幕位置。
