...
pub(crate) fn sync_token_keychain_service() -> String {
    sync_token_keychain_service_for(current(), 9)
}

/// The v2-v8 slots remain readable so an existing login is never discarded
...
mod tests {
    ...
    fn marker_identity_is_stable_and_scopes_keychain_and_instance_without_path() {
        ...
        assert_eq!(
            sync_token_keychain_service_for(&launch_profile, 9),
            format!("com.kunpeng.reader.sync-token.v9.isolated.{id}")
        );
        assert_eq!(
