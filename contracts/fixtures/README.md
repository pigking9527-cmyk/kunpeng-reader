...
16. `app-settings.v1.json` 固定 Windows、Linux、macOS 共享的非敏感软件设置实体、跳转回退图标 30–160 px 大小与保留的 1–10 级兼容镜像、主窗口菜单栏的 28–52 px 尺寸/完整排序/可选隐藏（设置不可隐藏）以及图标和文字的显示/内部顺序、资讯来源与自定义贴吧选择、书库问答偏好、最多 24 条自定义手势（每条 48 个归一化轨迹点）及提示设置，以及全局阅读排版/分页偏好，并验证未知字段保留。
