...
- `book_state_v2`：按图书内容 ID 保存阅读位置、书签、高亮、批注、评分等轻量状态。兼容字段 `tags`、`collections` 仅供旧客户端读取和离线核心迁移包 v1 携带；独立组织实体存在时不得作为权威来源。其可选 `progress_history` 是单本书的每日最后阅读位置摘要；条目字段为 Unix 秒 `at`、百分比 `progress`、章节 `chapter`、章节内比例 `frac` 和可选续读锚点 `position`。拉取或合并时，同一**本地日历日**仅保留 `at` 最大的条目，按日期升序保留最近至多 3650 日。缺失字段等同空历史；客户端写回完整书籍状态时必须保留已经收到的历史和未知 payload 字段。
...
- `ai_reader_history_v1`：单书智读记录以图书内容 SHA-256 为 ID；本机历史不设活跃记录上限，云端按账号跨书合计只保留最近 100 条；智读可选择最近自动同步或手动选择，提问、总结与脑图共用这一账号级额度。书库问答使用稳定 ID `library-v1`：本机历史同样不设活跃记录上限，设置可选 `off`（不同步）、`recent`（自动上传最近 100 条）或 `manual`（仅上传用户明确标记为云端的至多 100 条）。每类最多保留 200 条删除墓碑。每条历史记录有稳定条目 ID；删除时仅同步 `id` 与 `deletedAt` 条目墓碑，避免旧设备复活已删除记录。书库记录仅同步问题、回答和来源书名/章节/材料类型，不同步书籍原文片段、本机书籍 ID 或路径。该本地偏好及可选 `cloudSaved` 标记是协议 v1 的可选字段；旧客户端须忽略未知字段。
