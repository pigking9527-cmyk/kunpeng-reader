...
export type AsyncPhase =
  "idle" | "loading" | "success" | "failure" | "cancelled";
...
export type SensitiveActionPhase =
  "idle" | "confirming" | "running" | "failure" | "cancelled";
...
export interface AccountSyncState {
  ...
  readonly sensitive: {
    readonly phase: SensitiveActionPhase;
...
export type AccountSyncAction =
  ...
  | { readonly type: "sensitive-action-cancelled" }
  | {
      readonly type: "confirm-sensitive-action";
      readonly action: SensitiveAction;
    }
  | { readonly type: "dismiss-sensitive-action" }
  | {
      readonly type: "sensitive-action-started";
      readonly action: SensitiveAction;
    }
  | {
      readonly type: "sensitive-action-succeeded";
      readonly action: SensitiveAction;
    }
  | {
      readonly type: "sensitive-action-failed";
      readonly action: SensitiveAction;
      readonly notice: string;
    }
...
export function accountSyncReducer(
  ...
  switch (action.type) {
    ...
    case "sync-progress":
      if (
        !isCurrent(action.requestId, state.sync.requestId) ||
    ...
    case "sync-failed":
      ...
      return {
        ...
        sync: {
          ...state.sync,
          phase: action.offline ? "offline" : "failure",
          progress: null,
        },
    ...
    case "sync-cancelled":
      ...
      return {
        ...state,
        sync: { ...state.sync, phase: "cancelled", progress: null },
        notice: "同步已取消。",
      };
    case "sensitive-action-cancelled":
      return {
        ...state,
        sensitive: {
          ...state.sensitive,
    ...
    case "dismiss-sensitive-action":
      return {
        ...state,
        sensitive: { ...state.sensitive, phase: "idle", pendingAction: null },
        notice: "",
      };
