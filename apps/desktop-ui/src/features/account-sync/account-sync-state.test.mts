...
test("authentication state only accepts account metadata, never credentials or tokens", () => {
  const state = accountSyncReducer(initialAccountSyncState, {
    type: "auth-started",
    requestId: 1,
  });
  const authenticated = accountSyncReducer(state, {
    type: "auth-succeeded",
    requestId: 1,
    account,
  });
...
test("a stale successful sync cannot overwrite a later cancellation", () => {
  let state = accountSyncReducer(initialAccountSyncState, {
    type: "sync-started",
    requestId: 2,
  });
  state = accountSyncReducer(state, { type: "sync-cancelled", requestId: 2 });
  const stale = accountSyncReducer(state, {
    type: "sync-succeeded",
    requestId: 1,
    report,
  });
...
test("sync renders bounded progress and preserves offline/conflict as explicit states", () => {
  ...
  state = accountSyncReducer(state, {
    type: "sync-progress",
    requestId: 1,
    progress: { stage: "uploading", completed: 12, total: 10 },
  });
  ...
  state = accountSyncReducer(state, {
    type: "sync-conflict",
    requestId: 2,
    conflict: { kind: "conflict", count: 2 },
  });
...
test("sensitive operations require a separate confirmation state and reset account after cloud clear", () => {
  ...
  state = accountSyncReducer(state, {
    type: "confirm-sensitive-action",
    action: "clear-cloud",
  });
  assert.equal(state.sensitive.phase, "confirming");
  assert.equal(state.sensitive.pendingAction, "clear-cloud");
  state = accountSyncReducer(state, {
    type: "sensitive-action-started",
    action: "clear-cloud",
  });
  state = accountSyncReducer(state, {
    type: "sensitive-action-succeeded",
    action: "clear-cloud",
  });
...
test("a failed confirmation stays visible for retry and a cancellation dismisses it", () => {
  ...
  state = accountSyncReducer(state, {
    type: "sensitive-action-failed",
    action: "delete-account",
    notice: "操作未完成，请稍后重试。",
  });
  assert.equal(state.sensitive.phase, "confirming");
  assert.equal(state.sensitive.pendingAction, "delete-account");
  state = accountSyncReducer(state, {
    type: "sensitive-action-cancelled",
  });
  assert.equal(state.sensitive.phase, "cancelled");
  assert.equal(state.sensitive.pendingAction, null);
});
...
test("unsafe transport text is never selected as a user-facing error", () => {
  ...
  assert.equal(
    safeFailureNotice(error, "同步未完成，请稍后重试。"),
    "同步未完成，请稍后重试。",
  );
});
