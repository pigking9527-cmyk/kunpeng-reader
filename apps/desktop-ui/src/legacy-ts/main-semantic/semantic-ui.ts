...
export function installSemanticUi(target: unknown): SemanticUiGlobal | null {
  ...
  function init(options: SemanticUiOptions = {}) {
    ...
    const modelSelect = el<HTMLSelectElement>("sem-model-select");
    ...
    function render(payload: SemanticTaskCenter | SemanticViewProgress = {}) {
      ...
      const activeModel = String(
        progress.model_id || modelSelect?.value || "bge-small-zh-v1.5",
      ...
      if (modelSelect && progress.model_id)
        modelSelect.value = progress.model_id;
      ...
      const gpuDownloadTotal = Math.max(
        0,
        Number(gpuStatus?.runtime_download_bytes || 0),
      );
      const gpuDownloaded = Math.max(
        0,
        Math.min(
          gpuDownloadTotal,
          Number(gpuStatus?.runtime_downloaded_bytes || 0),
        ),
      );
      const gpuDownloadPercent = gpuDownloadTotal
        ? Math.round((gpuDownloaded * 100) / gpuDownloadTotal)
        : 0;
      const hasSavedGpuDownload =
        gpuDownloaded > 0 && !gpuStatus?.runtime_ready;
      if (gpuMeta && !gpuInstallRunning) {
        const hardwareMessage = gpuStatus?.runtime_ready
          ...
          : gpuStatus?.message ||
            semText(
              "semGpuInitial",
              "Select Recheck to read the local GPU status.",
            );
      ...
      if (gpuInstallButton) {
        ...
        gpuInstallButton.textContent = gpuInstallRunning
          ...
          : hasSavedGpuDownload
            ? semText(
                "semResumeGpuRuntime",
                "Resume GPU component installation ({percent}%)",
                { percent: gpuDownloadPercent },
              )
      ...
      if (vectorProgress)
        vectorProgress.hidden = vectorStatusChecking || !hasSemanticIndex;
      setProgressBar(
        vectorBar,
      ...
      if (rerankerDeleteButton)
        rerankerDeleteButton.disabled =
          busy || (!progress.reranker_downloaded && !progress.reranker_partial);
      if (m3BuildButton)
      ...
      if (vectorBuildButton)
        vectorBuildButton.textContent =
          vectorDone > 0 && !progress.semantic_ready
            ? semText("semResumeIndex", "Resume semantic index")
            : semText("semBuildIndex", "Build semantic index");
      if (vectorPauseButton)
        vectorPauseButton.textContent = semText("semPause", "Pause");
      if (vectorDeleteButton)
        vectorDeleteButton.textContent = semText("semDelete", "Delete");
      if (acceleratorBuildButton)
        acceleratorBuildButton.textContent = legacyAccelerator
          ? semText("semUpdateAccelerator", "Update accelerator index")
          : semText("semBuildAccelerator", "Build accelerator index");
      if (acceleratorDeleteButton)
        acceleratorDeleteButton.textContent = semText("semDelete", "Delete");
      if (multiProfileBuildButton)
      ...
      if (multiProfileDeleteButton)
        multiProfileDeleteButton.textContent = semText("semDelete", "Delete");
    ...
    async function installGpuRuntime() {
      ...
      const downloaded = Math.max(
        0,
        Math.min(total, Number(gpuStatus.runtime_downloaded_bytes || 0)),
      );
    ...
    on(modelSelect, "change", async () => {
      ...
      const current = semanticCache.get()?.model_id || "bge-small-zh-v1.5";
      const dimensions =
        MODEL_DIMENSIONS[next as keyof typeof MODEL_DIMENSIONS];
      const modelLabel =
        modelSelect.selectedOptions?.[0]?.textContent?.trim() || next;
      if (next === current) return;
      if (
        !confirmAction(
          "切换模型会使用一套独立的语义索引。切换后请下载新模型并重新建立语义索引；原模型的缓存会被保留。是否继续？",
        )
      ) {
        modelSelect.value = current;
        return;
      }
      const cachedNext = semanticCache.use(next);
      if (cachedNext) render(cachedNext);
      modelSelect.disabled = true;
      setStatus(
        semText(
          "semModelSwitching",
          "Switching to {model} ({dimensions} dimensions)…",
          { model: modelLabel, dimensions },
        ),
        "busy",
      );
      try {
        await semanticPort.selectModel(next);
