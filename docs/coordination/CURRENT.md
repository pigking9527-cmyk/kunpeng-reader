...
16. ADR-0021 定义 `app_settings_v1/default`：Windows、Linux、macOS 同步账户级非敏感软件设置；同步跳转回退开关、隐藏规则、30–160 px 的图标大小（并保留 1–10 级旧字段兼容镜像）和相对屏幕位置（0–1000 宽高千分比），以及资讯来源/自定义贴吧选择、书库问答的回答长度、历史同步策略、字号和 BGE-M3 长文精读偏好。资讯正文、文章缓存/URL、模型、索引和路径不进入该实体；客户端按字段补丁保留未知字段，旧客户端忽略该可选实体。
