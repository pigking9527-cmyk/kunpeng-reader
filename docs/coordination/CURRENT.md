...
| macOS 桌面包 | Apple Silicon 早期开发测试包可使用临时签名发布，须在发布说明中明确未公证。 | 早期测试包不等于稳定版或已通过 Gatekeeper 验收。 | 稳定版前完成 Developer ID 签名、公证及干净机 Gatekeeper 安装验收。 |
...
- 许可证整改：`v1.0.0` 至 `v1.16.0` 的桌面发行物曾链接 GPL-3.0 `epub` 依赖；当前开发版已切换为 Apache-2.0 `rbook` + 本仓库兼容适配层，并移除 vendored GPL 源码。仓库所有者于 2026-08-31 确认解析兼容、发行包第三方声明、历史资产处置与法律复核完成，允许恢复早期开发测试包发布；每次发布仍运行许可证与知识产权检查。稳定版平台签名与公证要求不变。详见 `docs/legal/historical-release-license-audit.md`。
