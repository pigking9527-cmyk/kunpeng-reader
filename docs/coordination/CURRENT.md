...
| macOS 桌面包 | Apple Silicon Beta 资产由受控 GitHub Actions 生成；Beta 1.1 经项目负责人明确批准可使用临时签名。 | 临时签名测试包不等于稳定版、Developer ID 签名、公证或无 Gatekeeper 提示。 | 稳定发布仍须完成 Developer ID 签名、公证及干净机 Gatekeeper 安装验收；临时签名路径只能目标为 GitHub prerelease。 |
...
- 许可证整改：历史版本曾链接 GPL-3.0 `epub` 依赖；当前新项目使用 Apache-2.0 `rbook` + 本仓库兼容适配层，并移除 vendored GPL 源码。项目负责人于 2026-08-27 明确确认历史版本已撤下、无存量用户并授权 Beta 1.1 公开预发布；许可证门禁已为当前新项目开启。详见 `docs/legal/historical-release-license-audit.md`。Beta 1.1 的 macOS 为明确标注的临时签名早期测试包，稳定版仍须签名与公证。
...
18. ADR-0029 保留 Rust 同步 API v4 的历史迁移记录；ADR-0031 将当前开发基线升级为 v5。正式 Rust 同步服务已完成受保护 PostgreSQL 迁移至 0036、对象存储配置、备份副本迁移演练与回环服务切换验证；既有 HTTPS `/v1/*` 由该服务承接。资讯发布只接受最终 publication bundle，30 天留存清理、设备投递与历史回源由服务端负责。客户端真实联调仍只可使用合成 fixture，并等待 DPAPI 安全输入的账户会话及短期 capability；已移除的 Python 服务不是 v5 兼容层或回滚实现。
