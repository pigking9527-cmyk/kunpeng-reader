...
| macOS 桌面包 | Apple Silicon Beta 资产由受控 GitHub Actions 生成。 | 本机安装或 CI 构建通过，不等于可跳过签名、公证或 Gatekeeper 验收。 | 完成 Developer ID 签名、公证及干净机 Gatekeeper 安装验收；工作流缺少凭据或失败时不上传替代包。 |
...
- 许可证整改：历史版本曾链接 GPL-3.0 `epub` 依赖；当前新项目使用 Apache-2.0 `rbook` + 本仓库兼容适配层，并移除 vendored GPL 源码。项目负责人于 2026-08-27 明确确认历史版本已撤下、无存量用户并授权 Beta 1.1 公开预发布；许可证门禁已为当前新项目开启。详见 `docs/legal/historical-release-license-audit.md`。macOS 仍不得绕过 Developer ID 签名与公证。
