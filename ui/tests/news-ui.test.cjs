...
test("News original-page return icon can be hidden while gesture close remains available", () => {
  ...
  assert.match(script, /hideReturnIcon: global\.ReaderExperimentalFeatures\?\.enabled\?\.\("newsnowHideReturnIcon"\) === true/);
  assert.match(backend, /pub hide_return_icon: bool/);
  assert.match(backend, /const hideReturnIcon = __KUNPENG_HIDE_RETURN_ICON__;/);
  assert.match(backend, /if \(hideReturnIcon \|\| document\.getElementById\("kunpeng-news-return"\)\) return;/);
  assert.match(backend, /也可以通过手势关闭页面/);
  assert.match(backend, /if request\.hide_return_icon\s*\{\s*"true"\s*\}\s*else\s*\{\s*"false"\s*\}/);
});
...
test("Gesture settings live in common settings and return from news, library, and reader", () => {
  ...
  assert.match(html, /id="gesture-settings-toggle"[^>]*aria-controls="gesture-settings-content"[\s\S]*?手势设置[\s\S]*?⌄/);
  assert.match(html, /id="gesture-editor-close"[^>]*aria-label="收起手势编辑器"/);
  assert.match(html, /id="gesture-settings-content"[^>]*hidden/);
  assert.match(html, /id="gesture-global-precision-toggle"[^>]*aria-controls="gesture-global-precision-settings"/);
  assert.match(html, /id="gesture-global-precision-settings"[^>]*hidden/);
  assert.match(html, /id="gesture-global-precision" type="range" min="1" max="10" step="1"/);
  assert.match(html, /id="gesture-hint-enabled" type="checkbox" role="switch"/);
  assert.match(html, /id="gesture-hint-settings-toggle"[^>]*>手势提示/);
  ...
  assert.match(styles, /\.gesture-disclosure \{[^}]*border: 0;[^}]*background: transparent/);
  assert.match(html, /id="gesture-action-choice"[\s\S]*?id="gesture-search"[\s\S]*?id="gesture-new"[\s\S]*?id="gesture-list"/);
  assert.match(html, /id="gesture-editor"[\s\S]*?id="gesture-editor-options"[\s\S]*?id="gesture-editor-title"[\s\S]*?id="gesture-pad"[\s\S]*?id="gesture-save"/);
  ...
  assert.match(html, /id="gesture-action-choice" class="gesture-choice-section" hidden/);
  assert.match(html, /id="gesture-editor-options" class="gesture-editor-options" hidden/);
  assert.match(html, /id="gesture-action-options" class="gesture-action-options"/);
  assert.match(html, /data-gesture-action="back"/);
  assert.match(html, /id="gesture-action-hint" class="gesture-auto-scope-note"/);
  assert.match(html, /id="gesture-action" type="hidden"/);
  assert.match(html, /id="gesture-test"/);
  assert.match(html, /id="gesture-precision-global"[^>]*value="global"/);
  assert.match(html, /id="gesture-precision-independent"[^>]*value="independent"/);
  assert.doesNotMatch(html, /id="gesture-settings-enabled"/);
  assert.match(html, /id="gesture-precision" type="range" min="1" max="10" step="1"/);
  assert.match(html, /<script src="news-ui\.js"><\/script>[\s\S]*?<script src="gesture-ui\.js"><\/script>/);
  ...
  assert.match(script, /gestureBack: \(\) => \{ if \(!reader\.hidden\) closeArticle/);
  assert.match(readerHtml, /<script src="news-gesture\.js"><\/script>[\s\S]*?<script src="reader-gesture\.js"><\/script>/);
  assert.match(readerGesture, /global\.closeReaderWindow\?\.\(\)/);
  assert.match(readerPage, /readerGestureDrawing=true;readerGestureSource=source;readerGesturePointerId=source==='pointer'\?e\.pointerId:null/);
  assert.match(readerPage, /document\.documentElement\.setPointerCapture\(e\.pointerId\)/);
  assert.match(readerPage, /readerGesture:\{phase:phase,x:e\.clientX,y:e\.clientY\}/);
  ...
  assert.ok(gestures.similarity(reference, reference.slice().reverse()) < gestures.matchThreshold("5"));
  const downThenRight = [{ x: 0, y: 0 }, { x: 0, y: 110 }, { x: 90, y: 110 }];
  const downThenShortRight = [{ x: 20, y: 10 }, { x: 20, y: 150 }, { x: 35, y: 150 }];
  const rightThenDown = [{ x: 0, y: 0 }, { x: 90, y: 0 }, { x: 90, y: 110 }];
  assert.deepEqual(gestures.directionSequence(downThenRight), gestures.directionSequence(downThenShortRight));
  assert.ok(gestures.similarity(downThenRight, downThenShortRight) >= gestures.matchThreshold("10"));
  assert.ok(gestures.similarity(downThenRight, rightThenDown) < gestures.matchThreshold("5"));
  assert.equal(gestures.similarity(downThenRight, [{ x: 0, y: 0 }, { x: 0, y: 8 }, { x: 5, y: 8 }]), 0);
  assert.equal(gestures.normalize([{ x: 0, y: 0 }, { x: 24, y: 24 }]).length, gestures.SAMPLE_COUNT);
...
test("Gesture feedback, reopen, and book-info actions are integrated across the shell and reader", () => {
  ...
  assert.match(gestureUi, /actionSearch\.addEventListener\("input", filterActionOptions\)/);
  assert.match(gestureUi, /source\.action === "book_info"/);
  assert.match(gestureUi, /source\.action === "reopen_last"/);
  assert.match(gestureUi, /HINT_SETTINGS_KEY/);
  assert.match(gestureUi, /enabled: saved\.enabled === true/);
  assert.match(gestureUi, /backgroundEnabled: saved\.backgroundEnabled !== false/);
  ...
  assert.match(gestureUi, /if \(!hintSettings\.backgroundEnabled\) return "transparent"/);
  assert.match(styles, /\.gesture-hint-background-control \{[^}]*align-items: center/);
  assert.match(styles, /\.gesture-hint-controls \{[^}]*grid-template-columns: minmax\(0, 1fr\) minmax\(0, 1fr\) auto/);
  assert.match(styles, /\.gesture-hint-background-control \{[^}]*justify-self: end/);
  ...
  assert.match(gestureUi, /runCloseOrReopen\(action, "手势设置", closeSettings, openSettings\)/);
  ...
  assert.match(gestureUi, /function runCloseOrReopen\(/);
  assert.match(styles, /\.gesture-settings-card \{[^}]*max-height: calc\(100dvh - 32px\);[^}]*overflow-y: auto/);
  assert.match(styles, /\.gesture-manager-card\.is-editor-open \.gesture-manager-layout \{ align-items: start; \}/);
  assert.match(styles, /\.gesture-manager-list-pane, \.gesture-editor \{ display: grid; align-content: start;/);
  assert.doesNotMatch(styles, /\.gesture-editor \{[^}]*overflow-y: auto/);
  assert.doesNotMatch(styles, /\.gesture-list \{[^}]*overflow: auto/);
  assert.doesNotMatch(styles, /\.gesture-action-options \{[^}]*overflow-y: auto/);
  assert.match(gestureUi, /global\.addEventListener\("pointermove", moveTraining, true\)/);
  assert.match(gestureUi, /global\.addEventListener\("pointerup", finishTraining, true\)/);
  const matcher = gestureUi.slice(gestureUi.indexOf("function matchProfile"), gestureUi.indexOf("function begin"));
  assert.doesNotMatch(matcher, /profile\.action === "back"/);
  assert.doesNotMatch(matcher, /profile\.scope === surface\.scope/);
  assert.doesNotMatch(matcher, /surface\.allowedActions\.includes\(profile\.action\)/);
  assert.match(readerGesture, /async function closeReaderSurface\(source\)/);
  assert.match(readerGesture, /root\.body\?\.classList\.contains\("ai-reader-open"\)/);
  assert.match(readerGesture, /global\.closeAiReaderSide\?\.\(\)/);
  assert.match(readerGesture, /function requestFrameSurfaceClose\(\)/);
  assert.match(readerGesture, /frame\.contentWindow\.postMessage\(\{ readerGestureAction: "back" \}, "\*"\)/);
  assert.match(readerGesture, /source === "frame" && await requestFrameSurfaceClose\(\)/);
  ...
  assert.doesNotMatch(readerGesture, /profile\.action !== "book_info" \|\| String\(global\.currentBookId/);
