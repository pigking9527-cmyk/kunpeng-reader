...
test("test intelligence workspace replaces the standalone World Monitor entry without changing news", () => {
  ...
  assert.match(html, /id="intelligence-workspace-page"[\s\S]*?id="intelligence-processing-summary"[\s\S]*?id="intelligence-local-model-base-url"[\s\S]*?id="intelligence-digest-list"[\s\S]*?id="intelligence-context-evidence"/);
  ...
  assert.match(html, /今日热点要点/);
  assert.match(html, /本机 Qwen 27B Q3/);
  assert.match(html, /仅接受本机 OpenAI 兼容服务/);
  assert.doesNotMatch(html, /intelligence-briefing-pipeline|不是逐条罗列资讯/);
  ...
  assert.match(html, /本机处理链路[\s\S]*?等待资讯/);
...
test("test intelligence workspace has one generated controller after the existing news controller", () => {
  ...
  assert.match(controller, /useRefreshCommand \? "newsnow_refresh" : "newsnow_list"/);
  ...
  assert.match(controller, /intelligence_generate_brief/);
  assert.match(controller, /intelligence_local_model_save/);
  assert.match(controller, /selectIntelligenceBriefCandidates/);
  assert.match(controller, /parseIntelligenceModelBriefs/);
  assert.match(controller, /selectedItem = null/);
  assert.match(controller, /openNews\.hidden = true/);
  assert.doesNotMatch(controller, /intelligence-digest-open|查看原文/);
  ...
  assert.match(styles, /\.intelligence-model-field\s*\{/);
  assert.match(styles, /\.intelligence-context-evidence\s*\{/);
  ...
  assert.match(styles, /\.intelligence-context-card p\s*\{[\s\S]*?overflow-wrap:\s*anywhere/);
