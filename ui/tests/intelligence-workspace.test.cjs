...
test("test intelligence workspace has one generated controller after the existing news controller", () => {
  ...
  assert.match(controller, /if \(sourceDirectoryOpen\) renderSourceDirectory\(\);/);
  assert.match(controller, /waitForWorkspaceFirstPaint/);
  assert.match(controller, /setStandardStatus\("正在打开情报中心…"\);[\s\S]*?await waitForWorkspaceFirstPaint\(\);/);
