...
const source = fs.readFileSync(path.join(root, "recovery-settings-snapshot.js"), "utf8");
...
test("recovery snapshots both WebView origins without copying credentials", () => {
  assert.match(source, /scope = global\.location\.pathname\.endsWith\("reader\.html"\) \? "reader" : "main"/);
  ...
  assert.match(main, /<script src="recovery-settings-snapshot\.js"><\/script>/);
  assert.match(reader, /<script src="recovery-settings-snapshot\.js"><\/script>/);
});
