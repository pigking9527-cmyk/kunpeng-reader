...
const semanticUiSource = fs.readFileSync(
  path.join(root, "..", "apps", "desktop-ui", "src", "legacy-ts", "main-semantic", "semantic-ui.ts"),
  "utf8",
);
...
test("smart management presents exactly three model responsibilities", () => {
  assert.match(html, /class="smart-model-grid"/);
  assert.match(html, />语义模型</);
  assert.match(html, /Qwen3 Embedding 0\.6B/);
  assert.match(html, />Agent 模型</);
  assert.match(html, />生图 \/ 视频模型</);
  assert.match(html, /id="sem-advanced-maintenance"/);
  assert.match(html, /id="sem-library-model-select"/);
  assert.match(html, /id="sem-library-model-apply"/);
  assert.doesNotMatch(html, /<option value="(?:bge-small-zh-v1\.5|bge-large-zh-v1\.5|multilingual-e5-small)">/);
  assert.doesNotMatch(html, /id="sem-solution-(?:standard|high|m3)"/);
  assert.match(html, /id="sem-reranker-download"/);
  assert.match(html, /id="sem-m3-build"/);
  assert.match(html, /id="sem-accel-build"/);
  assert.match(html, /id="sem-multi-build"/);
  assert.match(semanticUiSource, /saveIntelligenceModel/);
  assert.doesNotMatch(semanticUiSource, /qwen27CompositeChoice|qwen27DeepSearch|semanticQwen27DeepSearchV1/);
  assert.match(html, /id="sem-media-primary"/);
  assert.match(html, /id="sem-media-start"/);
  assert.match(html, /id="sem-media-choose-comfy"/);
  assert.match(html, /id="sem-media-choose-workflow"/);
  assert.match(html, /id="sem-media-save-comfy"/);
  assert.match(html, /__KUNPENG_PROMPT__/);
  assert.match(semanticUiSource, /dialogsFromTauriGlobal\(global\)\.open/);
  assert.match(semanticUiSource, /configureReaderMediaComfyUi/);
  assert.doesNotMatch(html, /id="sem-media-download"[^>]*>下载并准备本地模型/);
  assert.doesNotMatch(html, /id="sem-model-select"/);
  assert.match(html, /data-i18n="semTitle">智能管理/);
  assert.match(html, /<div id="sem-semantic-title" class="sem-title">语义模型/);
  assert.match(html, /H3 只在通过内存、显存与磁盘检查后才允许安装/);
  assert.doesNotMatch(html, /完整语义检索|一键启用/);
});

test("smart management keeps one compact summary and no parallel task pages", () => {
  assert.doesNotMatch(html, /data-sem-tab=/);
  assert.doesNotMatch(html, /class="sem-capability-tabs sem-workspace-tabs"/);
  assert.match(html, /id="sem-status" class="ai-status sem-task-strip"/);
  assert.equal((html.match(/id="sem-status"/g) || []).length, 1);
  assert.match(html, /id="sem-overview-summary"/);
  assert.match(html, /按推荐方案配置/);
  assert.match(html, /id="sem-agent-primary"/);
  assert.match(html, /class="sem-storage-tools"/);
  assert.match(html, /id="sem-advanced-maintenance"/);
  assert.match(styles, /\.smart-model-grid/);
  assert.match(styles, /\.sem-task-strip/);
});

test("each smart-model card keeps its own settings and gesture explanations", () => {
  const semanticCard = html.slice(
    html.indexOf('id="sem-semantic-title"'),
    html.indexOf('id="sem-agent-title"'),
  );
  const agentCard = html.slice(
    html.indexOf('id="sem-agent-title"'),
    html.indexOf('id="sem-media-title"'),
  );
  const mediaCard = html.slice(
    html.indexOf('id="sem-media-title"'),
    html.indexOf('</div>\n    </div>\n    <div id="external-dict-modal"'),
  );
  assert.match(semanticCard, /语义模型设置与维护/);
  assert.match(semanticCard, /id="sem-library-model-select"/);
  assert.match(semanticCard, /id="sem-retrieval-mode"/);
  assert.match(semanticCard, /id="sem-reranker-download"/);
  assert.match(semanticCard, /id="sem-m3-build"/);
  assert.match(agentCard, /Agent 模型设置与维护/);
  assert.match(agentCard, /id="sem-device-policy"/);
  assert.match(agentCard, /id="sem-understanding-preflight"/);
  assert.match(agentCard, /id="sem-qwen27-card"/);
  assert.match(mediaCard, /生图 \/ 视频模型设置与维护/);
  assert.match(mediaCard, /id="sem-media-policy"/);
  assert.match(mediaCard, /id="sem-media-choose-comfy"/);
  assert.doesNotMatch(html, /高级与维护（模型、检索、设备、ComfyUI 与可重建数据）/);
  for (const id of [
    "sem-semantic-primary",
    "sem-library-model-apply",
    "sem-vector-build",
    "sem-reranker-download",
    "sem-agent-primary",
    "sem-understanding-preflight",
    "sem-qwen27-preflight",
    "sem-media-primary",
    "sem-media-download",
    "sem-media-policy",
    "sem-media-choose-workflow",
  ]) {
    assert.match(
      html,
      new RegExp(
        `(?:id="${id}"[\\s\\S]{0,360}?data-gesture-info=|data-gesture-info=[\\s\\S]{0,360}?id="${id}")`,
      ),
      `${id} needs an information-gesture explanation`,
    );
  }
  assert.match(styles, /\.smart-model-card > \.sem-card-settings/);
  const gestureUi = fs.readFileSync(
    path.join(repoRoot, "apps", "desktop-ui", "src", "legacy-ts", "gesture-ui", "gesture-ui.ts"),
    "utf8",
  );
  assert.match(gestureUi, /function gestureInfoForTarget/);
  assert.match(gestureUi, /\[data-gesture-info\]/);
  assert.match(gestureUi, /openGestureInfo\(info\)/);
});

test("Agent card opens the existing secure API editor in a focused cloud-Agent mode", () => {
  const apiSettings = fs.readFileSync(
    path.join(repoRoot, "apps", "desktop-ui", "src", "legacy-ts", "main-settings", "api-settings-ui.ts"),
    "utf8",
  );
  assert.match(html, /id="sem-agent-primary"/);
  assert.match(html, /id="api-ai-purpose-picker"/);
  assert.match(html, /data-ai-purpose="reading"/);
  assert.match(html, /data-ai-purpose="library"/);
  assert.match(html, /data-ai-purpose="other"/);
  assert.match(html, /id="api-ai-key"[^>]*type="password"/);
  assert.match(semanticUiSource, /data-agent-config-mode/);
  assert.match(apiSettings, /云端 Agent 配置/);
  assert.match(apiSettings, /ai-reader-profiles-changed/);
  const cloudConfigHandler = semanticUiSource.slice(
    semanticUiSource.indexOf("function openAgentCloudConfiguration"),
    semanticUiSource.indexOf("async function refreshCapabilityRoutes"),
  );
  assert.doesNotMatch(cloudConfigHandler, /apiKey|localStorage|sessionStorage/);
  assert.match(styles, /data-agent-config-mode="true"/);
});
...
test("semantic model keeps the recommended capability and the original retrieval controls", () => {
  assert.match(html, /Qwen3 Embedding 0\.6B/);
  assert.match(html, /负责智能搜索、相似内容和相关推荐/);
  assert.match(semanticUi, /"qwen3-embedding-0\.6b"/);
  assert.match(html, /id="sem-retrieval-mode"/);
  assert.match(html, /id="sem-retrieval-m3-option"/);
  assert.doesNotMatch(html, /id="sem-m3-long-context"/);
  assert.match(html, /data-i18n="semGpu"/);
  ...
  assert.match(semanticUi, /active_model_device_label/);
  assert.match(semanticUi, /semActiveDevice/);
  assert.match(i18n, /semActiveDevice: "当前模型实际运行于 \{device\}。"/);
...
test("technical reranking remains available in the original advanced section", () => {
  assert.match(html, /id="sem-reranker-download"/);
  assert.match(html, /id="sem-reranker-delete"/);
...
test("an undownloaded model presents its expected download size", () => {
  ...
  assert.match(semanticUi, /downloadEstimate: "2\.8 GB"/);
  assert.match(semanticUi, /downloadEstimate: "610 MB"/);
  assert.match(semanticUi, /downloadEstimate: "4\.4 GB"/);
  assert.match(semanticUi, /downloadEstimate: "450 MB"/);
  assert.match(semanticUi, /semModelNotDownloaded/);
  assert.match(semanticUi, /modelDownloadButton\.hidden =\s*!!progress\.model_downloading/);
});
...
test("an empty semantic index hides its progress track and delete action", () => {
  ...
  assert.match(semanticUi, /vectorProgress\.hidden =\s*!!progress\.model_downloading \|\| vectorStatusChecking \|\| !hasSemanticIndex/);
  assert.match(semanticUi, /!hasSemanticIndex\s*\? semText\("semNotBuilt"/);
  assert.match(semanticUi, /const vectorDeleteAvailable =\s*!progress\.model_downloading/);
  assert.match(semanticUi, /vectorDeleteButton\.hidden = !vectorDeleteAvailable/);
...
test("semantic indexing identifies active GPU acceleration beneath the book progress", () => {
  ...
  assert.match(semanticUiSource, /active_model_device/);
  assert.match(semanticUiSource, /startsWith\("cuda"\)/);
  assert.match(semanticUiSource, /vectorGpuMeta\.hidden = !gpuIndexing/);
  assert.match(semanticUiSource, /semGpuIndexing/);
  assert.match(i18n, /semGpuIndexing: "GPU 加速索引中"/);
  assert.match(styles, /\.sem-index-acceleration/);
});

test("model download keeps its own readable progress and never borrows completed index progress", () => {
  assert.match(html, /id="sem-model-download-progress"/);
  assert.match(html, /id="sem-model-download-bar"/);
  assert.match(html, /id="sem-model-download-label"/);
  ...
  assert.match(semanticUi, /正在下载语义模型：\$\{modelDownloadPercent\}%/);
  assert.match(semanticUi, /else if \(progress\.model_downloading\)\s*setStatus\(/);
  assert.match(semanticUi, /progress\.building && vectorTotal/);
  ...
  assert.match(modelRust, /fn qwen_downloaded_bytes\(alias: &str, file: &str\) -> u64/);
  assert.match(modelRust, /Qwen3-Embedding-0\.6B-Q8_0/);
});

test("advanced model details retain vector dimensions while capability switch feedback stays plain", () => {
  ...
  assert.match(semanticUi, /正在准备“\$\{label\}”的本地搜索库/);
  assert.match(semanticUi, /当前智能搜索继续可用/);
...
test("semantic management restores cached state then detects all three model cards", () => {
  ...
  assert.ok(open.indexOf("semanticCache.get()") < open.indexOf("refreshWorkspace()"));
  assert.match(semanticUi, /async function refreshWorkspace\(\)/);
  assert.match(semanticUi, /refreshGpuStatus\(\)/);
  assert.match(semanticUi, /refreshIntelligenceModel\(\)/);
  assert.match(semanticUi, /refreshReaderMediaStatus\(\)/);
  assert.match(semanticUi, /refreshAgentProfiles\(\)/);
  assert.match(
    semanticUi,
  ...
  assert.match(semanticUi, /install_semantic_gpu_runtime/);
  assert.doesNotMatch(semanticUi, /semGpuInstallConfirm/);
  assert.match(semanticUi, /semanticPort\.tasks\(reconcile\)/);
  assert.match(
    semanticUi,
    /semanticPort\.selectSolution\([\s\S]*?next,[\s\S]*?await refresh\(true\);/,
  );
...
test("semantic status stays usable while exact metadata verification runs", () => {
  ...
  assert.doesNotMatch(semanticUi, /semanticCache\.use\(next\)/);
  assert.match(semanticUi, /progress\.solution_switching/);
  assert.match(semanticUi, /当前智能搜索继续可用/);
...
test("smart search library exposes coverage, incremental rebuild, and device policy controls", () => {
  assert.match(html, /id="sem-library-plan"/);
  assert.match(html, /id="sem-library-coverage"/);
  assert.match(html, /id="sem-library-pending"/);
  assert.match(html, /id="sem-vector-rebuild"/);
  assert.match(html, /id="sem-device-policy"/);
  assert.match(html, /value="auto"/);
  assert.match(html, /value="gpu"/);
  assert.match(html, /value="cpu"/);
  assert.match(semanticUi, /semUpdateLocalContent/);
  assert.match(semanticUi, /semRebuildAll/);
});

test("semantic settings retain catalog-managed visible controls and refresh when the app language changes", () => {
  for (const key of ["semTitle", "semGpu", "semUpdateLocalContent", "semRebuildAll", "semClearSearchData"]) {
    assert.match(html, new RegExp(`data-i18n(?:-aria)?="${key}"`));
  }
  assert.match(i18n, /ja: \{\s*semTitle: "インテリジェント設定"/);
  assert.match(i18n, /ko: \{\s*semTitle: "지능형 설정"/);
