...
test("about exposes shared bug and feature feedback editor", () => {
  ...
  assert.match(about, /class="modal-head"><span[^>]*data-i18n="about"[^>]*>关于<\/span>/);
  assert.doesNotMatch(about, /ℹ️/u);
  assert.match(html, /id="about-feedback-bug"[^>]*>提交 Bug</);
  assert.match(html, /id="about-feedback-feature"[^>]*data-i18n="suggestFeature"[^>]*>功能建议</);
  ...
  assert.match(html, /id="feedback-attach-problem-trace"[^>]*data-i18n="attachTrace"[^>]*>附到本次反馈（推荐）</);
  assert.match(html, /id="feedback-save-problem-trace"[^>]*data-i18n="saveTraceDesktop"[^>]*>保存问题记录到桌面</);
