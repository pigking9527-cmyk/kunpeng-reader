...
(function exposeSyncUi(global) {
  ...
  function init(options = {}) {
    ...
    if (typeof invoke !== "function")
      throw new Error("ReaderSyncUI.init 缺少 invoke");
    ...
    const syncResetNewPasswordEl = document.getElementById(
    ...
    const accountPasswordFormEl = document.getElementById(
    ...
    function setSyncButtonState(
      ...
      if (state === "syncing")
        setConnectionState("checking", "serviceCommunicating");
    ...
    function applyAccountUsage(status = {}) {
      ...
      if (accountStorageNoteEl)
        accountStorageNoteEl.textContent = "同步实体和恢复版本会计入此额度";
    ...
    syncResetRequestBtn.addEventListener("click", async () => {
      try {
        await invoke("auth_request_password_reset", {
          request: {
            url: "",
        ...
        setResetStatus("发送验证码失败：" + error, "error");
      }
    });
    syncResetConfirmBtn.addEventListener("click", async () => {
      try {
        const res = await invoke("auth_confirm_password_reset", {
          request: {
            url: "",
            username: syncUsernameEl.value.trim(),
        ...
        syncSettingsLoaded = true;
        updateAccountView({
          username: res.user?.username || syncUsernameEl.value,
        });
        ...
        setResetStatus("重置失败：" + error, "error");
      }
    });
    ...
    syncUsernameEl.addEventListener("input", () => {
      ...
      if (q) {
        savedAccountsEl
          .querySelectorAll(".saved-account-item")
          .forEach((row) => {
