(function (global) {
  ...
  const launchAtLoginRow = document.getElementById("startup-enhancement-autostart-row");
  const launchAtLogin = document.getElementById("startup-enhancement-autostart");
  ...
  let config = { enabled: false, continueHighCost: false, launchAtLogin: false, launchAtLoginAvailable: false };
  ...
  function save(next) {
    ...
    render();
    return invoke("set_startup_enhancement_config", { request: config }).then((saved) => {
  ...
  launchAtLogin?.addEventListener("change", () => save({ ...config, launchAtLogin: launchAtLogin.checked }).catch(() => {}));
  ...
  global.ReaderStartupEnhancement = {
    backgroundWorkAllowed: () => (
      (!backgrounded || config.continueHighCost) && Date.now() >= highCostResumeAtMs
    ),
    highCostRetryDelay: () => (
