...
(function (global) {
  ...
  if (!modal || !managerCard || !managerLayout || !settingsToggle || !settingsContent || !globalPrecisionToggle || !globalPrecisionSettings || !gear || !enabledInput || !newButton || !search || !list || !editor || !editorTitle || !editorClose || !editorOptions || !nameInput || !actionChoice || !actionHint || !actionInput || !actionOptions || !actionSearch || !actionEmpty || !inputInput || !precision || !precisionValue || !globalPrecisionInput || !globalPrecisionValue || !precisionGlobalMode || !precisionIndependentMode || !precisionGlobalHint || !hintEnabled || !hintSettingsToggle || !hintSettingsPanel || !hintFontSize || !hintFontSizeValue || !hintBackgroundEnabled || !hintBackground || !hintOpacity || !hintOpacityValue || !hintPreview || !hintPreviewArea || !pad || !save || !clear || !test || !status || !trail) return;
  ...
  function normalizeProfile(value, index) {
    ...
    const action = source.action === "book_info" ? "book_info" : (source.action === "reopen_last" ? "reopen_last" : "back");
  ...
  function saveProfiles() {
    try { global.localStorage?.setItem?.(MANAGER_KEY, JSON.stringify({ version: 3, globalPrecision, profiles })); } catch (_) { /* local preference */ }
    syncLegacyGesture();
    publish();
  }
  ...
  function compatibleProfile() {
    const usable = profiles.filter((profile) => profile.enabled && profile.action === "back" && profile.points.length);
    return usable.find((profile) => profile.scope === "reader") || usable.find((profile) => profile.scope === "global") || usable[0] || null;
  }
  ...
  function actionLabel(value) { return ({ back: "返回／关闭当前页", book_info: "打开图书信息", reopen_last: "重新打开上一个页面" })[value] || "返回／关闭当前页"; }
  ...
  function filterActionOptions() {
    ...
    const action = ["book_info", "reopen_last"].includes(actionInput.value) ? actionInput.value : "back";
    ...
    actionHint.textContent = action === "book_info"
      ? "手势会在所有页面参与匹配；书架会优先打开手势起点所在图书，阅读页会直接打开当前图书信息。"
      : action === "reopen_last"
        ? "手势会在所有页面参与匹配；可恢复由返回手势关闭的上一个页面，尚无可恢复页面时不会执行。"
  ...
  function renderList() {
    ...
    const visible = profiles.filter((profile) => {
      const haystack = (profile.name + " " + actionLabel(profile.action) + " 自动适用").toLocaleLowerCase();
    ...
    visible.forEach((profile) => {
      ...
      meta.textContent = "自动适用 · " + actionLabel(profile.action) + " · " + (profile.precisionMode === "global" ? "全局精度 " + globalPrecision : "独立精度 " + profile.precision) + (profileHasPath(profile) ? "" : " · 未录制");
  ...
  function openEditor(profile) {
    const source = profile ? { ...profile, points: profile.points.slice() } : {
      id: makeId(), name: "返回／关闭当前页", scope: "global", action: "back", input: "mouse-right", enabled: true, points: [], precisionMode: "global", precision: globalPrecision,
    };
    editing = source;
  ...
  function conflictFor(profile) {
    return profiles.find((other) => other.id !== profile.id && other.action === profile.action && profileHasPath(other) && api.similarity(other.points, profile.points) >= api.matchThreshold(Math.max(Number(effectivePrecision(other)), Number(effectivePrecision(profile)))));
  }
  function saveEditor() {
  ...
  function deleteProfile(id) {
    ...
    if (!profiles.length) profiles = [{ id: makeId(), name: "返回／关闭当前页", scope: "global", action: "back", input: "mouse-right", enabled: false, points: [], precisionMode: "global", precision: globalPrecision }];
    saveProfiles();
    if (editing?.id === id) closeEditor();
    renderList();
  }
  const closedPages = [];
  function rememberClosedPage(name, reopen) {
    if (typeof reopen !== "function") return;
    closedPages.push({ name: String(name || "上一个页面").slice(0, 48), reopen });
  ...
  function runCloseOrReopen(action, name, close, reopen) {
    ...
    rememberClosedPage(name, reopen);
    close();
  }
  ...
  function fallbackSurface(target) {
    ...
    if (bookInfo?.classList.contains("show") && bookInfo.contains(target)) {
      return { allowedActions: supportedActions(["back"]), onMatch: (action) => runCloseOrReopen(action, "图书信息", () => root.getElementById("book-info-close")?.click(), () => bookInfo.classList.add("show")) };
    }
  ...
  function matchProfile(surface, points) {
    const candidates = profiles.filter((profile) => profile.enabled && profileHasPath(profile) && canApplyAction(surface, profile.action));
    let best = null;
  ...
  function previewMatch(gesture) {
    const matched = matchProfile(gesture.surface, gesture.points);
    if (!matched) {
  ...
  function cancelGestureKeepHint() {
    ...
    if (gesture.points.length > 1) suppressContextMenuUntil = Date.now() + 500;
    clearTrail();
    if (matched && canApplyAction(gesture.surface, matched.profile.action)) showHint(matched.profile.name);
  }
