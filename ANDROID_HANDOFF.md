...
- 单本“图书信息”页可编辑书名、作者、简介、0–5 分评分、标签和书单；标签/书单支持逗号批量新增、删除、复用书架已有项，保存时去空白、去重、保留顺序，当前 Android v0.3.0 仍沿用既有 `book_state_v2` 兼容字段；按 ADR-0016 升级前可继续同步阅读状态，但其标签/书单编辑不会覆盖服务器上的独立权威实体。书架“更多工具 > 标签管理”提供桌面同等的用户标签用量统计、全书架改名和删除：改名始终遍历完整书库（不受当前筛选影响）、合并大小写等价标签并去重；删除须二次确认，且只移除该标签，绝不删除图书、文件或 AI 分类标签。
...
- 同步进度、书签、高亮、批注、评分、标签、书单、生词本、阅读统计及单书每日位置摘要 `book_state_v2.progress_history`；用户标签和收藏夹须迁移为 `user_book_tags_v1`、`book_collections_v1` 独立权威实体，`book_state_v2` 仅保留兼容镜像；不上传图书文件、正文、封面原图、路径、模型或语义索引。Android 拉取后按本地日仅保留时间较晚的位置、最多 3650 日，随后写回时保留已收历史和未知 payload 字段，不得以空数组覆盖桌面时间线。
...
- `book_state_v2`：阅读位置、书签、高亮、批注、评分等轻量状态；标签、书单字段仅为兼容镜像。
- `vocab`：生词本。
...
- 2026-08-07 PDF 选区菜单迁移：Android 原生 PDF 长按选字现补齐桌面同类的“网页搜索”和“以选文添加书签”。仅允许复制的文档显示这两项；两者均要求同页、非空且不超过 2,000 字符，跨页/超长会明确拒绝。搜索仅由用户点击后经既有 HTTPS 外部打开桥接发往 Baidu，选文不写偏好或同步；书签继续使用稳定 pdf:page:N 页锚点，选文仅成为 24 rune 截断的标签，避免伪造 EPUB 文本坐标。PDF 选区单测、PDF 高亮几何和外链桥接共 9 项通过，定向 flutter analyze 无问题；Gradle :app:assembleDebug 成功。当前 Play Debug APK：C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk，334,958,581 bytes，SHA-256 4A8C6B2983AEBAF237251486C105147F520683320AD3A9C77E5527C07804DD0B，已在 API 35 x86_64 模拟器覆盖安装并冷启动（3.871 秒），近期日志未见该应用 FATAL EXCEPTION。空书架模拟器没有可选文字 PDF，尚未执行真实长按选字、系统浏览器或 PDF 书签跳转的端到端验收；也尚未验证 ARM64 真机。

- 2026-08-07 重复内容导入对齐：Windows 的“重复组安全合并”针对旧数值书架 ID；Android 的 books.content_id 从一开始就是完整 SHA-256 主键，私有对象也按 SHA 命名，正常数据模型中无法同时存在同内容的两条记录。因此没有照搬会破坏约束的合并按钮；改为在手动/自动导入结果中明确显示新增数和跳过的重复内容数。跳过时既有图书、阅读状态和标注保持权威，不会新建同步实体或改动契约。仓储回归覆盖同 SHA 的再次导入不能覆盖原路径、进度、字数、手工标签、书单、书签、高亮或 book_state_v2 时钟；仓储及导入结果定向回归 18 项通过。

- 2026-08-07 阅读问题追踪反馈附件迁移：文本阅读器的“阅读问题记录”页新增“附到新的 Bug 反馈”。只有用户点按并在二次确认后才重新冻结闭集追踪，并作为单个 UTF-8 JSON 内存附件打开新的 Bug 草稿；不经 SAF、不落盘、不进入诊断/同步/恢复点，提交前可移除，只有点击提交才会经已配置 HTTPS 服务发送，且仍要求 acceptedAttachments 明确确认。Feature 草稿会忽略初始附件。追踪页确认路径、反馈草稿零自动请求、附件确认与既有反馈服务的定向回归共 21 项通过；完整 flutter test 为 507 项通过、1 项 Edge 实网探针默认跳过。Gradle :app:assembleDebug 成功；Play Debug APK 为 C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk，334,961,646 bytes，SHA-256 84552BF26B804A184C42E4EC0FD58ECD9C794B836C71DDF730552198A2C9A081，已在 API 35 x86_64 模拟器覆盖安装并冷启动 4.107 秒，近期日志未见该应用 FATAL EXCEPTION。空书架模拟器没有真实阅读器追踪事件可操作，尚未真实点击“追踪→Bug 草稿”路径或提交到 HTTPS 服务，未验证 ARM64 真机。

- 2026-08-07 EPUB 目录当前小节迁移：Windows 已按当前 DOM fragment 精确选中目录；Android 原先只按 chapterIndex，导致同一物理章节的多条目录同时高亮。现仅把当前章最多 256 个、每项最多 512 字符的已解析 fragment 交给受 CSP 保护的 WebView，运行时用 document.getElementById 和当前 UTF-16 可见起点解析“已越过的最后一个”锚点，并以短 fragment 回传本机 UI，目录只选择唯一匹配项；不存在、无效或不可用的 WebView 则回退为该章第一条，保持旧行为。fragment 不作为 CSS/JS 代码执行，不落库、不写阅读位置、不进入同步、迁移、日志或网络。定向 analyze 无问题，目录追踪和 WebView 回归 17 项通过；全量 flutter test 为 511 项通过、1 项默认跳过；Gradle :app:assembleDebug 成功。当前 Play Debug APK：C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk，334,966,605 bytes，SHA-256 78E32826E267A7AAC4B8F02CDBEFD980CC27EACD2A9D681354A8A5CA0A0A4093，Debug 签名；已在 API 35 x86_64 模拟器 emulator-5554 覆盖安装并冷启动 MainActivity（4.983 秒）。空书架模拟器尚未以含同章多 fragment 的真实 EPUB 走目录端到端验收，仍需该样本及 ARM64 真机验证。
- 2026-08-07 四边独立阅读边距迁移：Windows 已提供上/下/左/右独立边距；Android 原先只可合并调“左右”“上下”。现保留两条成组滑块作为手机快捷调节，并在可展开的“分别设置边距”中提供左、右、上、下四条本地滑块，简中/繁中/英文均有文案。旧本机 presentation JSON 缺少四边字段时，会把已有 horizontalPadding/verticalPadding 安全映射到左右/上下；新字段均限 0–80 px，未知/非法值回退或夹值。四边距只进入本机 metadata 与 WebView CSS，不改变阅读 locator、正文、书签高亮、同步、迁移、日志或网络。普通滚动的底部仍保留视口缓冲；横屏双页按左右边距计算列宽、按上下边距计算可用高度，避免不对称边距裁切。静态 analyze 无问题；presentation 与 WebView 定向回归 20 项通过；全量 flutter test 为 513 项通过、1 项默认跳过；Gradle :app:assembleDebug 成功。当前 Play Debug APK：C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk，334,973,712 bytes，SHA-256 C82A96A4878A9FF32ED5BD002090651357C3CE3D89FC49C0E8ACA1AB772B0B2A，Debug 签名；已在 API 35 x86_64 模拟器 emulator-5554 覆盖安装并冷启动 MainActivity（4.182 秒）。空书架模拟器未实际打开 EPUB/TXT，仍需在真实内容上手调四边、切换书本样式/分页单页/双页、旋转和重开后验证视觉与阅读位置；仍未验证 ARM64 真机。
- 2026-08-08 界面语言扩展：Android 设置现与 Windows 已公开语言选项对齐，可选“跟随系统 / 简中 / 繁中 / English / 日本語 / 한국어 / Français / Deutsch / Español / Русский / Português (Brasil)”。新增值仍仅写本机 `ui.app_locale.v1`，即时重建界面，不进入阅读位置、书籍、批注、高亮、同步、迁移、日志或网络。新增日/韩/法/德/西/俄/葡资源并保留所有既有 ARB 键：应用壳、书架、阅读器/PDF 高频文案已翻译，尚未逐项人工校对的低频独立页安全回退英文，不能称作七语全界面完成。`flutter gen-l10n`、针对 controller/设置页的 analyze 与 11 项回归均通过；完整 `flutter test -r compact` 为 514 项通过、1 项默认跳过；正确 Android Gradle 根目录的 `:app:assembleDebug` 成功（现有 `flutter_open_chinese_convert` KGP 未来兼容警告仍在）。Debug 签名 Play APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk`，335,304,068 bytes，SHA-256 `26CF4FAC48F01AC3F8C7C5D10487691E877908EB8CA2D3B2BFFFDDBE1FB640AB`；已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动。该模拟器书架为空，尚未逐项人工切换新增语言后打开真实 EPUB/PDF，也未做 ARM64 真机验收。
- 2026-08-08 文内已保存高亮操作迁移：Windows 读到高亮时可直接操作；Android 现同时覆盖 CSS Highlight API 与 DOM mark fallback。WebView 只会对当前已渲染的本机高亮发送 `{type:"highlight-tap",index}`；index 必须在已渲染集合中，CSS 路径以 caret/range 命中，DOM 路径用运行时 WeakMap 而非书内属性，Flutter 只按原始 `book_state_v2.highlights` 数组槽位读取，非法/过期/跨章索引一律忽略，不信任或回传正文、偏移、路径。点按后可复制、网页搜索、查词、翻译、生成书摘、跨书搜索、语义搜索、智读、改色、编辑批注和删除；全部复用既有用户触发工具及高亮同步写入，不新增字段或协议。定向 analyze 无问题、桥接/高亮管理 21 项通过；全量 `flutter test -r compact` 为 515 项通过、1 项默认跳过；`android :app:assembleDebug` 成功。Play Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk`，335,308,424 bytes，SHA-256 `0F160BB5205AC27770741187B0069F5B4F2A7052F90998C76DD122580AAEEB8C`，已在 API 35 x86_64 `emulator-5554` 覆盖安装和冷启动。空书架模拟器未能手动验证真实 EPUB/TXT 的 CSS/fallback 两条点按路径，仍需真实内容与 ARM64 真机验收。- 2026-08-08 阅读页书籍信息迁移：Android 阅读器工具菜单新增“书籍信息”面板，显示书名、作者、格式、文件大小、已读字数、标签和书单；可调整 0–5 分评分及编辑简介，保存时沿用既有 `BookRecord` 更新与同步路径。该面板不显示本机源路径，也不新增同步字段、协议或联网行为。`flutter analyze lib/reader/reader_screen.dart` 无问题；完整 `flutter test -r compact` 为 515 项通过、1 项默认跳过；`android :app:assembleDebug` 成功。Play Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\play\debug\app-play-debug.apk`，335,309,531 bytes，SHA-256 `1146C74A58E9B5BC1A614CA4AFA26DDA03854FF90049C003CA4D32027529BA4F`，已在 API 35 x86_64 `emulator-5554` 以 `adb install -r` 覆盖安装并冷启动。空书架模拟器未能在真实图书中逐项点开工具菜单、修改信息并验证同步；仍需真实内容与 ARM64 真机验收。

- 2026-08-08 阅读器字数与朗读重建：Android 文本阅读器已按 ADR-0007 接回可见页有效字数，满足门槛的回退重读会再次计入，短停留、快速翻页、后台、章节加载和浮层不会计入；“书籍信息”可显式计算并本机缓存全书非空白字符数，始终与累计有效阅读字数分开。朗读工具栏现支持系统离线语音的逐句 UTF-16 高亮、停止、切章节/后台/退出清理；Edge 朗读默认关闭，只有在本机设置页面勾选知情开关后才会经固定 TLS 主机发送正在朗读的当前一句，网络、协议、大小或播放失败会回退离线语音，正文、音频、语音偏好和同意状态均不进入同步实体。还恢复了阅读器路由传入的翻译与语义搜索接口，选文翻译重新使用应用内 BYOK 翻译页。`dart analyze lib/reader/reader_screen.dart lib/reader/reader_tts_preferences.dart test/reader/reader_tts_preferences_test.dart` 无问题；TTS 游标、Edge 输入边界、偏好、全书字数和重读字数定向 22 项通过；`android :app:assembleDebug` 成功。Full Debug APK 已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动到 `MainActivity`，近期日志未见 FATAL EXCEPTION。模拟器书架为空，尚未以真实正文手工验证离线发声、当前句范围、跨章和 Edge 实网回退；仍需内容样本与 ARM64 真机验收。

- 2026-08-08 阅读呈现与书内搜索接线：Android 文本阅读器现将本机 `ReaderPresentation` 全量传给受 CSP 保护的 WebView：可切换滚动/翻页、单页/宽屏双页、主题、黑体/衬线、原书/本机样式、行距、段距、字距、四边距和可访问性保护下的淡入/滑动动效；这些偏好只写本机 `reader.presentation.v1`，不改变正文、UTF-16 locator、标注或同步实体。重排后重新按当前 locator restore，并使用 WebView 实测当前章节的页码/总页数和严格布局签名缓存；没有把只测当前章伪称为全书页数。书内搜索工具现扫描本机已解析章节（UTF-16、懒加载、上限/截断、PDF 明确不支持），结果可跨章跳转；选中结果等待新章节恢复后以独立 `kp-inline-` 前缀精确高亮，不影响书架传入的外部检索高亮，关闭时只清除自身。`dart analyze` 无问题；呈现、页数缓存、动效、HTML 与书内搜索定向 29 项通过；`android :app:assembleDebug` 成功。Full Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\full\debug\app-full-debug.apk`，336,503,835 bytes，SHA-256 `3FDDC42779967690580BB4A7389116FD1A89AA9C0AF84C56378C49958220681F`，已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动至 `MainActivity`，近期无 FATAL EXCEPTION。模拟器书架为空，尚未用真实 EPUB/TXT 手动验收分页/双页、边距重排续读、章节测页、跨章搜索和 72+ 同词命中的视觉路径；全书后台逐章测量需要独立测量 WebView worker，仍未实现，ARM64 真机也未验收。
- 2026-08-08 目录、书签与标注管理接线：文本阅读器目录当前项现以章节内 fragment 精确定位；仅传入当前章节受长度和数量限制的片段，WebView 回传值仍由本机白名单复核，无法定位时安全回退本章首项。工具菜单新增“书签与标注”管理页：书签可按既有 locator 打开或二次确认删除；高亮按阅读位置倒序显示，可定位、编辑批注、逐项四色改色或二次确认删除。排序展示与原始 `book_state_v2.highlights` 数组槽位分离，编辑不会误改同章的另一条标注；不新增同步字段、契约、网络传输或正文外发。`dart analyze lib/reader/reader_screen.dart lib/reader/bookmark_management.dart lib/reader/highlight_management.dart lib/reader/reader_toc_tracking.dart` 无问题；目录、书签、高亮与 WebView 定向 27 项通过；`android :app:assembleDebug` 成功（现有 flutter_open_chinese_convert Kotlin Gradle Plugin 未来兼容警告仍在）。Full Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\full\debug\app-full-debug.apk`，336,516,799 bytes，SHA-256 `992AA7909634D8F87CBFA3088DC06F48492FE094BD72C48290793A12EFC8C86A`，已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动至 `MainActivity`，近期日志未见 FATAL EXCEPTION。模拟器书架为空，尚未以含同章多个目录锚点、真实书签与标注的 EPUB/TXT 执行端到端手测，亦未验证 ARM64 真机。
- 2026-08-08 沉浸阅读与有效时长接线：Android 文本阅读器现接入本机 `reader.immersive_mode.v1`：普通模式工具栏常驻，开启“沉浸阅读”后才允许正文中部点击显隐；加载、朗读、选区、批注和阅读浮层会保持控制入口可见。该偏好只写本机 metadata，不改变 locator、正文、统计同步实体或网络。此前 `readingSeconds` 误按打开阅读页后的整段墙钟时间累计，现改用既有 `ReadingTimeCredit`：只有收到有效可见位置、处于前台且未被加载/选区/批注/目录/搜索/书籍信息/显示设置/朗读设置/词典/翻译/智读/生词本/书摘等覆盖时才启动；每 15 秒结算一次、单次最多 20 秒，后台、章节切换和离开会立即丢弃未完成 tick，恢复后必须等待新的可见位置，字数统计同步使用同一暂停边界。未改 contracts 或同步字段；仓储既有增量 bucket 路径继续保存正确的 seconds/words delta。`dart analyze` 无问题；有效时长、重读字数、沉浸模式、HTML 与呈现定向 40 项通过（新增连续可见位置不重置时长 tick 用例）；`android :app:assembleDebug` 成功。Full Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\full\debug\app-full-debug.apk`，336,521,850 bytes，SHA-256 `F572867532F43AA90D5C549C78DEEA97FE9A7235ED583BDD2429F4CA2640324F`，已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动，近期无 FATAL EXCEPTION。模拟器书架为空，尚未用真实正文手动验证 15 秒有效停留、后台/浮层暂停、恢复后重新激活和沉浸中部点击，亦未验证 ARM64 真机。
- 2026-08-08 书摘图片 SAF 导出接线：文本选文的“书摘”现在显示本机 Flutter 预览卡，可复制完整选文；导出图片只取 `ReaderExcerpt` 的最多 1200 UTF-16 字符（不拆代理对，界面明确提示截断）。卡片不再对该已声明上限作第二次静默行数截断；渲染后受 2 MP、PNG 签名和 2 MiB 双重限制，再通过 `StorageBridge.savePngDocument` 触发 Android ACTION_CREATE_DOCUMENT。跨 native 边界只传已检查 PNG 字节和净化文件名；不传书籍路径、WebView data URL、私有文件路径或返回 URI，取消不会提示成功，异常只显示通用错误。书摘及导出数据不进 metadata、book_state、同步、迁移、日志或网络。`dart analyze lib/reader/reader_screen.dart lib/reader/reader_excerpt_image.dart lib/platform/storage_bridge.dart` 无问题；图片边界、渲染卡及 SAF bridge 定向 26 项通过；`android :app:assembleDebug` 成功。Full Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\full\debug\app-full-debug.apk`，336,529,026 bytes，SHA-256 `0904BEC331502970D7B14F13120F788BC52B3C5D9AA2036984077EBAC95F046B`，已在 API 35 x86_64 `emulator-5554` 覆盖安装并冷启动，近期无 FATAL EXCEPTION。模拟器书架为空，尚未在真实 EPUB/TXT 上走选文、预览、取消/实际 SAF 保存、长文本/emoji、旋转或 ARM64 真机验收。
- 2026-08-08 读完推荐阅读器接线：文本阅读器现复用既有本机 `SemanticIndexService.recommendSimilarBooks` 与 `PostReadRecommendationTrigger`。阅读进度达到默认 90% 时最多发起一次本机预取；最后一章保持可读，只有末尾再向前翻页才在已有结果时显示至多 5 本推荐。缺模型、索引未完成、候选书不足、空结果或本机查询错误均静默跳过，不发网络请求，不同步正文、向量、索引、偏好或推荐结果。当前仍需把推荐项点击接到顶层既有打开图书路由；真实验收须至少两本完整索引的 EPUB/TXT。`dart analyze` 无问题，`post_read_recommendations_test` 4 项通过；`android :app:assembleDebug` 成功。
- 2026-08-08 读完推荐打开路由与外接键盘翻页：推荐卡现在返回受信任的本机 `BookRecord`，并由顶层既有 `_openBook` 路由打开，保留解析、阅读位置和返回栈处理；不直接拼接路径、同步或联网。正文 WebView 补齐与 Windows 一致的硬件键：PageDown/Right/Down/Space 向前，PageUp/Left/Up/Shift+Space 向后；Ctrl/Meta/Alt、输入法组合、contenteditable 与 INPUT/TEXTAREA/SELECT 均不接管。仅在正文 WebView 已获取焦点时生效，尚未把全局 Flutter 快捷键强加给 PDF、文本框或滑块，避免双重翻页。定向 `dart analyze` 无问题，HTML 键盘桥接 17 项加读完推荐 4 项共 21 项通过。Full Debug APK：`C:\Users\pigki\Documents\Codex\2026-07-21\claude-projects\kunpeng-reader-mobile\build\app\outputs\apk\full\debug\app-full-debug.apk`，336,540,496 bytes，SHA-256 `6AA483AB6A3CF876CE8864029CFBE12CFD45A557D5467FF09DFB635C1855240E`；已在 API 35 x86_64 `emulator-5554` 覆盖安装、冷启动至 MainActivity，近期日志未见应用 FATAL EXCEPTION。模拟器书架为空，仍需以真实 EPUB/TXT 配外接键盘和至少两本完整索引图书验证端到端路径，尚未验证 ARM64 真机。